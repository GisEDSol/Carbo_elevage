...
Description de l'arborescence des fichiers et le fonctinnement avec GitHub et 
...
# Etapes pour la prise en main du projet

Pour prendre en main cette méthode de travail, voici un exemple d'application.

## Importation du projet sur GitHub

Ici, décrire la procédure pour importer le projet dans un dépôt personnel
Adresse:

```{r}
# Création du dépot git en local (ajout des fichiers)
cd /media/sf_GIS_ED/Dev/Scripts/
git init
git add trunk/*
git commit -m "Ajout des fichiers et des réperoires de travail"

# Connexion vers le dépôt local
git remote add origin https://github.com/Rosalien/GISEDSol.git
git remote -v
```
