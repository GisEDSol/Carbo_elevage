...
- Installation des paquets postgreSQL/postGIS et du logiciel pgadmin3 pour visualiser les tables
...
L'importation de la base est réalisée avec la commande suivante, dans l'arborescence contenant le fichier `sol_elevage.sql` :
...
Database = sol_elevage # A garder Nom de la base de données
Servername = localhost # A garder Nom du serveur en local
Username = ****** # Changer en fonction du nom d'utilisateur
Password = ****** # Changer en fonction du mdp défini par l'utilisateur
Port=5432 # A garder Port de la base de données
...
```
Une fois lancé, les différents scripts du dépôt peuvent être utilisé. C'est l'objet de l'étape 4
