...
L'objectif de ce document est de fournir les informations nécessaires pour utiliser la base de données appelée `sol_elevage`. Le document présente l'organisation des données et les explications pour son exploitation.
...
```{r,evaL=TRUE,echo=FALSE}
metatable <- read.csv(paste(repmetadonnees,"Nomenclature_variables_bdat.csv",sep=""),sep=",")
pander(metatable,caption = "Description des champs des tables de la BDAT",justify = c('left', 'left'),split.cells = c("35%", "65%"),use.hyphening = TRUE)
```
