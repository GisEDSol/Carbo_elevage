...
# Paramètres
p_variable <- "var_sth7910" #variable à spatialiser
l_variable <- "Evolution de la STH 1979-2010 (%) "#label de la variable
couleur <- "Spectral"#nom de la palette couleur (selon RColorBrewer)display.brewer.all() pour connaître les différentes palettes

# Lancement des instructions ggplot2
gpclibPermit()
...
p1 <- ggplot(carto, aes(x=long, y=lat)) +  geom_polygon(data=carto, aes(group=group, fill=fill)) + 
  geom_path(data=carto, aes(x=long,y=lat,group=group),color="grey30")+
  scale_fill_brewer(palette = couleur,name=l_variable,guide = guide_legend(reverse=TRUE,nrow=1))+theme(legend.position="bottom")+
  scale_x_continuous("", breaks=NULL)+scale_y_continuous("", breaks=NULL)

ggsave(p1, file = paste(repsortie,"map_",p_variable,".png",sep=""), width = 7, height = 7)  
