...
tablecarto <- "dm_vecteurs.canton" 
period <- c("1979","1988","2000","2010") #Périodes de temps prise en compte
variablecarto <- c("p_prairie","p_sth","p_sfp","p_mf","p_c","p_cop")#variable(s) à cartographier
#variablecarto <- "p_prairie"
l_legend <- c("Prairie/SAU (%)","STH/SAU (STH)", "SFP/SAU (%)", "MF/SAU (%)","Céréale/SAU (%)","COP/SAU (%)")# Nom des labels associés aux variables à cartographier. Vecteur utilisé pour les titres des cartes
...
for(v in variablecarto){
  cpt <- cpt + 1
  nomfichier <- paste(v,style_classe,"_2",sep="")
  l_legendvaria <- l_legend[cpt]
  variableperiod <- paste(v,period,sep="")

  carto(dsn,tablecarto,variableperiod,nclasse,style_classe,couleur,l_legend=l_legendvaria,repsortie,nomfichier,dept=FALSE,reg=FALSE,nrowlayout=2,ncollayout=2,position="bottom",ggsaveheight=8,ggsavewidth=8)  
}

## Variation 
tablecarto <- "dm_vecteurs.canton"  #Nom de la table utilisée pour la cartographie (table postgis)
variablecarto <- "var_prairie1979_2010"
l_variable <- "Evolution prairie/SAU entre 1979 et 2010 (%)"
nomfichier <- "var_prairie1979_2010"
