...
p_fr <- bdatdiff %>% 
          ...
          ggplot(aes(x = factor(1), y = perc, fill = factor(evol_med),label=paste(perc*100,"%",sep=""))) +
...
p[[1]] <- bdatdiff %>% 
          group_by(zonage_cplt,evol_med) %>% 
          summarise(count=n()) %>% 
          mutate(perc=round((count/sum(count)),2)) %>%

          ggplot(aes(x = factor(zonage_cplt), y = perc, fill = factor(evol_med),label=paste(perc*100,"%",sep=""))) +
          scale_fill_brewer(palette = 'Paired',name="Evolution des teneurs en CO")+
          geom_bar(stat="identity", width = 0.7) +
          geom_text(size = 3, position = position_stack(vjust = 0.5))+
...
p[[2]] <- bdatdiff %>% 
          group_by(typo_clim,evol_med) %>% 
          summarise(count=n()) %>% 
          mutate(perc=round((count/sum(count)),2)) %>%

          ggplot(aes(x = factor(typo_clim), y = perc, fill = factor(evol_med),label=paste(perc*100,"%",sep=""))) +
          scale_fill_brewer(palette = 'Paired',name="Evolution des teneurs en CO")+
          geom_bar(stat="identity", width = 0.7) +
          geom_text(size = 3, position = position_stack(vjust = 0.5))+
...
bdatdiff$evol_med <- as.factor(bdatdiff$evol_med)
...
histo_period <- ggplot(bdatdiff,aes(evol_med)) + geom_bar() + facet_wrap(~ period)+
...
bdatdiff_classe <- melted.bdatdiff[complete.cases(melted.bdatdiff$evol_med) & melted.bdatdiff$evol_med !="I",]
...
bdatdiff_classe$evol_med <- as.factor(bdatdiff_classe$evol_med)

# Histogramme des valeurs 
histo_regionadmin <- ggplot(bdatdiff_classe,aes(evol_med,fill=nom_region)) + geom_bar(position="dodge") + facet_wrap(~ period) + 
...
variablecarto <- c("evol_med_13","evol_med_23","evol_med_35","evol_med_45") #variable à spatialiser
l_legend <- "Evolution des teneurs en CO"#label de la variable
nclasse <- 4 #Nombre de classes de valeurs pour la cartographie
...
for(s in periodetud){
        y <- paste("diff_med_",s,sep="")
        teststats <- paste("evol_med_",s,sep="")
        x <- paste("med_corgox_",nomdiff[nomdiff$Code %in% s,"Periode1"],sep="")
...
for(s in reg_elevage){
    print(s)
    d_typo <- dcast.bdat[dcast.bdat$zonage_simple %in% s,]
    p <- list()
    r2 <- list()
    
    for(i in periodetud){
      print(i)
        y <- paste("diff_med_",coname,"_",i,sep="")
        teststats <- paste("evol_med_",coname,"_",i,sep="")
        x <- paste("med_corgox_",nomdiff[nomdiff$Code %in% i,"Periode1"],sep="")
        xyNames <- c(x,y,teststats)

        #dd <- d_typo[complete.cases(d_typo[,xyNames]),][,xyNames]    
        dd <- d_typo[complete.cases(d_typo[,xyNames]) & d_typo[,teststats] %in% "D",][,xyNames]    
        if(nrow(dd) < 15){
...
for(x in periodetud){
  diffname <- paste("diff_med_pourcent_",x,sep="")
...
# Voir pour tester plusieurs combinaisons de variation d'occupation du sol (périodes) (sachant que les évolutions entre 1988-2000 pour la période 15 explique bien certaines tendances)
...
for(x in nomdiff$Code){
  ...
  for(i in ra_occupyears){
    ...
    wilcotest <- generate_label_df(db=datatest,value="diff_med_pourcent",lev=i,letters,position=0.5)

    lablex <- ""#legendx[cpt]
    labley <- "Différences des teneurs en CO (g/kg)"
  
    p[[i]] <- ggplot(datatest,aes_string(y="diff_med_pourcent",x=i))+geom_boxplot() + coord_flip() + geom_hline(yintercept = 0,color="red") + geom_text(data = wilcotest, vjust=-0.75,aes(x = plot.labels, y = V1, label = labels)) + scale_x_discrete(lablex)+scale_y_continuous(labley)+theme_perso()+labs(title=paste(x,"_",i,sep=""))
  }
tt <- do.call(grid.arrange,p)
ggsave(tt,file = paste(repsortie,"boxplot_diff_occup_",x,"_pourcent.png",sep=""), width = 17, height = 10)  
}
