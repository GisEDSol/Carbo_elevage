...
# graphique intéressant à creuser. Revoir en calculant les différences relatives et revoir pour les autres occupations du sol

bdatdiff <- subset(melted.bdatdiff,diff_corgox!="Non significatif")
periodetud <- c("14","24","15","25","13","35")
bdatdiff <- bdatdiff[complete.cases(bdatdiff$classe_var_sfp1988_2010) & bdatdiff$period %in% periodetud,]
...
#bdatdiff2 <- bdatdiff[bdatdiff$period %in% "13", ]
#bdatdiff2$diff_corgox <- as.factor(bdatdiff2$diff_corgox)
#bdatdiff2 <- bdatdiff2[complete.cases(bdatdiff2$diffmedian_corgox),]
#bdatdiff$classe_var_prairie1970_2010 <- ordered(bdatdiff$classe_var_prairie1970_2010, levels = c("[-97.8,-21.4]","(-21.4,-11.8]","(-11.8,-4.19]","(-4.19,4.5]","(4.5,73]"))
...
melted.bdatdiff$diffmedian_corgox <- as.numeric(melted.bdatdiff$diffmedian_corgox)
melted.bdatdiff <- melted.bdatdiff[complete.cases(melted.bdatdiff$diff_corgox),]
bdatdiff <- bdatdiff[complete.cases(bdatdiff$classe_argi_med),]
bdatdiff$classe_argi_med <- ordered(bdatdiff$classe_argi_med, levels = c("[20,150]","(150,182]","(182,240]","(240,562]"))


test <- bdatdiff[bdatdiff$zonage_simple %in% "B",]
test <- test[complete.cases(test$diffmedian_corgox),] 
# Revoir le test, problème avec boxplot.df <- ddply(db, lev, function (x) quantile(x$value,position,na.rm=TRUE) + 0.2)s sur les NA

wilcotest <- generate_label_df(db=test,value="diffmedian_corgox",lev="classe_var_sfp1988_2010",letters,position=0.5)

ggplot(bdatdiff,aes(y=diffmedian_corgox,x=classe_var_sfp1988_2010))+
 geom_boxplot()  + xlab("Variation de la SFP 1988-2010 (%)")+ylab("Différences des teneurs en CO")+ coord_flip() + geom_hline(yintercept = 0,color="red") +geom_text(data = wilcotest, vjust=-0.75,aes(x = plot.labels, y = V1, label = labels))
...
ggplot(bdatdiff,aes(y=diffmedian_corgox,x=classe_var_sfp1988_2010))+
 geom_boxplot() + facet_wrap(~ zonage_simple)  + xlab("Variation de la SFP 1988-2010 (%)")+ylab("Différences des teneurs en CO")+ coord_flip() + geom_hline(yintercept = 0,color="red")# stat_summary(fun.y=median, geom="line",size = 1,aes(group=1)) #scale_fill_brewer(palette = 'Paired',name="Evolution des prairies % (1970-2010)")


ggplot(bdatdiff,aes(y=diffmedian_corgox,x=classe_var_prairie1988_2010))+
 geom_boxplot() + facet_wrap(~ zonage_simple)  + xlab("Variation des prairies entre 1988-2010 (%)")+ylab("Différences des teneurs en CO")+ coord_flip() + geom_hline(yintercept = 0,color="red")# stat_summary(fun.y=median, geom="line",size = 1,aes(group=1)) #scale_fill_brewer(palette = 'Paired',name="Evolution des prairies % (1970-2010)")
                 # 
