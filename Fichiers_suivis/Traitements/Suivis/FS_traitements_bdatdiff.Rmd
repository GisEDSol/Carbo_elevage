...
histo_table <- count(bdatdiff_classe,c("diff_med","nom_region","period"))
...
for(s in periodetud){
        ...
        periode_co=periodetud
        stats <- array(NA, dim = c(3, 2),list(Diff = c("D","A","I"), mod = c("r2","coeff")))

        for(i in c("D","A","I")){        

          dd <- dcast.bdat[complete.cases(dcast.bdat[,xyNames]) & dcast.bdat[,teststats] %in% i,][,xyNames]    

          #dd <- dcast.bdat[complete.cases(dcast.bdat[,xyNames]) & dcast.bdat[,teststats] != "I",][,xyNames]    
          ...
          stats[i,"r2"] <- summary(lm(dd[,y]~dd[,x]))$r.squared
          stats[i,"coeff"] <- summary(lm(dd[,y]~dd[,x]))$coefficients[,"Estimate"][[2]]
        }

      #stats_r2 <- as.data.frame(cbind(melt(r2),melt(coeff_direc)))
      stats_r2 <- as.data.frame(stats)
      stats_r2$Type <- rownames(stats_r2)
      rest[[s]] <- stats_r2      
...
ggplot(test2, aes(x=Period,y=r2,colour=Type)) + geom_boxplot() +
...
for(s in reg_elevage){
    ...
    for(i in periodetud){
      print(i)
        y <- paste("diff_med_",i,sep="")
        teststats <- paste("evol_med_",i,sep="")
...
p <- ggplot(datatest,aes_string(y="diff_med",x=occupsol
))+
...
for(i in occup){
  ...
  wilcotest <- generate_label_df(db=datatest,value="diff_med",lev=i,letters,position=0.5)

  lablex <- ""#legendx[cpt]
  labley <- "Différences des teneurs en CO (g/kg)"
  
  p[[i]] <- ggplot(datatest,aes_string(y="diff_med",x=i
))+
...
for(i in reg_elevage){
  ...
  wilcotest <- generate_label_df(db=datatest,value="diff_med",lev=occupsol,letters,position=0.5)
  ...
  p[[i]] <- ggplot(datatest,aes_string(y="diff_med",x=occupsol
))+geom_boxplot() + coord_flip() + geom_hline(yintercept = 0,color="red") + geom_text(data = wilcotest, vjust=-0.75,aes(x = plot.labels, y = V1, label = labels)) + scale_x_discrete(lablex)+scale_y_continuous(labley)+theme_perso()+labs(title=i)
}
...
for(i in reg_elevage){
  ...
  wilcotest <- generate_label_df(db=datatest,value="diff_med",lev="classe_var_mf1988_2010",letters,position=0.5)
...
for(i in reg_elevage){
  ...
  wilcotest <- generate_label_df(db=datatest,value="diff_med",lev="classe_var_prairie1988_2010",letters,position=0.5)
...
for(i in reg_elevage){
  ...
  wilcotest <- generate_label_df(db=datatest,value="diff_med",lev="classe_p_prairie2000",letters,position=0.5)
...
for(i in reg_elevage){
  ...
  wilcotest <- generate_label_df(db=datatest,value="diff_med",lev="classe_var_sfp1988_2010",letters,position=0.5)
...
ggplot(bdatdiff,aes(y=diff_med,x=classe_var_mf1988_2010))+
