...
L'objectif de ce travail est d'établir un état des lieux de l'évolution des teneurs en carbone organique sur les sols cultivés au sein des différentes régions d'élevage et d'analyser ces évolutions au regard des changements d'occupation du sol liés à l'élevage.

# Données

## BDAT

Voir pour si c'est pertinent de présenter les effecifs dans cette partie...
...
- F2 : Zone pastorale méditerranéenne
...
```{r cdf_fr,highlight=TRUE,echo=FALSE,eval=TRUE,warning=FALSE,error=FALSE,fig.height=5, fig.width=7,fig.align="center"}
...
p <- ggplot(melted.bdat,aes(x=annees,y=value)) + 
     ...
     theme_perso(position="right")+ geom_text(data = test1,hjust = 2, vjust=-0.75,aes(x = plot.labels, y = V1, label = labels))+ geom_text(data = test2,hjust = -2,vjust=-0.75, aes(x = plot.labels, y = V1, label = labels)) + scale_fill_manual(values=c("#F1ED0AE6", "#F36E15E6"), name="Statistiques cantonales") + theme(legend.position="bottom")#+ coord_cartesian(ylim = ylim1)
p
```
...
Ici, on recommence uniquement pour les principales grandes régions d'élevage
