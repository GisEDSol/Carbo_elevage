...
repmaster <- "/media/jb/ec0c5059-c127-4ca9-9a18-28515f593aa2/Carbo_elevage/"
repdata <- "/media/jb/ec0c5059-c127-4ca9-9a18-28515f593aa2/Sauvegarde_GISED/GIS_ED/Dev/Data/"
...
reppapier <- "/media/jb/ec0c5059-c127-4ca9-9a18-28515f593aa2/Sauvegarde_GISED/GIS_ED/Com/Papier_carboelevage/Relectures_v1/Correction/Fig/" #répertoire de sortie pour la rédaction de l'article
...
# Travail uniquement sur les médianes
...
#tabledata <- read.csv("/media/sf_GIS_ED/Dev/ /Donnees_papier.csv")
...
p[["2"]] <- ggplot(datasummary2, aes(ymin = StartDate, ymax = EndDate, x = Count)) + geom_linerange(aes(colour=value),size=3) + coord_flip() + scale_colour_gradient2(low="red",mid="grey",high=muted("blue"),name="Taux d'évolution annuel (%)",breaks=c(round(min(datasummary2$value),1),0,round(max(datasummary2$value),1)),labels=c(round(min(datasummary2$value),1),0,round(max(datasummary2$value),1)),limits=c(round(min(datasummary2$value),1),round(max(datasummary2$value),1))) + ylab("Périodes") + xlab("Code des périodes comparées") + theme_perso() + geom_text(aes(fontface=2,y = StartDate, x = Count, label = ifelse(pvalue<=0.05,paste(round(value,1),"% **",sep=""),paste(round(value,1),"%",sep=""))), hjust = 0, vjust = 0.5, size = 3.5) #+ labs(title= paste(i)) ## Dans scale_colour_gradient2 : breaks=c(round(min(datasummary2$value),1),round(max(datasummary2$value),1))
#}
...
p <- ggplot(melted.bdat_regelevage) +
            ...
            scale_fill_manual(values=couleur,name="") + scale_x_discrete("Grandes régions d'élevage")+scale_y_continuous("Teneur en carbone organique (g/kg)")+ theme_perso("") + geom_text(data = wilcotest, vjust=-0.75,aes(x = plot.labels, y = V1, label = paste(labels,"/",Nbr_analyse,sep="")),size=5)#Ici, légère augmentation de la taille de la police
...
evol_elevage <- ggplot(meltstats, aes(ymin = StartDate, ymax = EndDate, x = Count)) + geom_linerange(aes(colour=value),size=3) + facet_wrap(~variable) + coord_flip() + scale_colour_gradient2(limit = c(min(meltstats$value),max(meltstats$value)),low="red",mid="grey",high=muted("blue"),name="Taux d'évolution annuel (%)") + ylab("Périodes") + xlab("") + theme_perso() + geom_text(aes(fontface=2,y = StartDate, x = Count, label = ifelse(pvalue<=0.05,paste(round(value,1),"% **",sep=""),paste(round(value,1),"%",sep=""))), hjust = 0, vjust = 0.5, size = 3.5)# + theme(legend.position="none")
evol_elevage
...
# Valeur absolu
label_legend <- c((round(min(meltstats$value),1)+round(min(meltstats$value)*0.01,1)),0,round(max(meltstats$value),1))
#label_legend <- c("Min","0","Max")
evol_elevage <- ggplot(meltstats, aes(ymin = StartDate, ymax = EndDate, x = Count)) + geom_linerange(aes(colour=value),size=3) + facet_wrap(~variable) + coord_flip() + scale_colour_gradient2(low="red",high=muted("blue"),mid="grey",name="Taux d'évolution annuel (%)",breaks=c((round(min(meltstats$value),1)+round(min(meltstats$value)*0.01,1)),0,round(max(meltstats$value),1)),labels=label_legend,limits=c((round(min(meltstats$value),1)+round(min(meltstats$value)*0.1,1)),round(max(meltstats$value),1))) + ylab("Périodes") + xlab("Code des périodes comparées") + theme_perso() + geom_text(aes(fontface=2,y = StartDate, x = Count, label = ifelse(pvalue<=0.05,paste(round(value,1),"% **",sep=""),paste(round(value,1),"%",sep=""))), hjust = 0, vjust = 0.5, size = 3.5)# + theme(legend.position="none")
evol_elevage

# Valeur relative à la surface des régions d'élevage (100000 km2)
evol_elevage_surface <- ggplot(meltstats, aes(ymin = StartDate, ymax = EndDate, x = Count)) + geom_linerange(aes(colour=value_surface ),size=3) + facet_wrap(~variable) + coord_flip() + scale_colour_gradient2(limit = c(min(meltstats$value_surface ),max(meltstats$value_surface )),low="red",high=muted("blue"),mid="grey",name="Taux d'évolution annuel (%)") + ylab("Périodes") + xlab("Code des périodes comparées") + theme_perso() + geom_text(aes(fontface=2,y = StartDate, x = Count, label = ifelse(pvalue<=0.05,paste(round(value_surface ,1),"% **",sep=""),paste(round(value_surface ,1),"%",sep=""))), hjust = 0, vjust = 0.5, size = 3.5) + theme(legend.position="none")
evol_elevage_surface
...
for(i in Variay){
  ...
  # Construction de la variable "type"
...
# Test d'endogénéité (REVOIR)
# selon http://eclr.humanities.manchester.ac.uk/index.php/IV_in_R
library(AER,lmtest)
endo <- c("p_sfp2000","p_sfp2010","p_prairie2000","p_prairie2010","p_mf2000","p_mf2010","p_sth2000","p_sth2010","p_c2000","p_c2010")
...
# Instrument (teneur en carbone sur deux périodes antérieures aux occupations du sol(<2000))
instru <- c("med_corgox_9599","med_corgox_9094")
instruplus <- paste(instru,collapse="+")

vNames <- unique(c("med_corgox_0509",instru,variamodel))
d <- dcast.bdat[complete.cases(dcast.bdat[vNames]),vNames]
...
for(i in endo){
  ...
  instrHausWutest <- waldtest(first_stage,.~.-med_corgox_9599-med_corgox_9094)
...
#print(xtable(rest),include.rownames=FALSE)
```
...
test2$positionperc <- as.numeric(c(rep("82",10),rep("49",32)))
#test3 <- unique(test2[,c("period","type","type2","perc","perctype")])

###################################

#couleur <- c("#60C233E6","#394AA7E6","#F79A39E6","#F1ED0AE6")

# Changement des couleurs basée sur une palette couleur adaptée aux daltoniens http://www.cookbook-r.com/Graphs/Colors_(ggplot2)/#a-colorblind-friendly-palette
colorblind <- c("#009E73","#56B4E9","#D55E00","#F0E442")
couleur <- colorblind
...
scale_colour_manual(values = c('Naturelle' = 'red', 'Anthropique' = 'blue'),name="Origine des variables") +
...
bdatdiff$period <- as.factor(bdatdiff$period)
levels(bdatdiff$period) <- list("9094-0004"="13","9094-1014"="15","0004-1014"="35")
...
p[[1]] <- bdatdiff %>% 
          ...
          theme_perso() + labs(title="a") + coord_flip() + theme(axis.text.y = element_text(angle = 90, hjust = 0.5))
p[[1]]
```
...
#bdatdiff <- melted.bdatdiff[melted.bdatdiff$zonage_simple !="H" & melted.bdatdiff$period %in% c("13","15","35"),]
bdatdiff$zonage_simple <- with(bdatdiff, factor(zonage_simple, levels = rev(levels(zonage_simple))))
...
p[[2]] <- bdatdiff %>% 
          ...
          theme_perso() + labs(title="b") + coord_flip()
...
## Même type de graphique qu'en haut mais avec le pourcentage de SAU qui évolue.

p2 <- list()
p2[[1]] <- bdatdiff %>% 
          group_by(period,evol_med) %>% 
          summarise(sau=sum(sau_2010,na.rm=TRUE),count=n()) %>% 
          mutate(perc=(round((count/sum(count)*100),1)),perc_sau=(round((sau/sum(sau)*100),1))) %>%
          ggplot(aes(x = factor(period), y = perc_sau, fill = factor(evol_med),label=paste(perc_sau,"%",sep=""))) +
          scale_fill_manual(values=couleur,name="Evolution des teneurs en CO")+
          geom_bar(stat="identity", width = 0.7) +
          geom_text(size = 3, position = position_stack(vjust = 0.5))+
          xlab("Périodes comparées")+ylab("Pourcentage de SAU (%)")+
          theme_perso() + labs(title="a") + coord_flip() + theme(axis.text.y = element_text(angle = 90, hjust = 0.5))
...
p2[[2]] <- bdatdiff %>% 
          ...
          summarise(sau=sum(sau_2010,na.rm=TRUE),count=n()) %>% 
          mutate(perc=(round((count/sum(count)*100),1)),perc_sau=(round((sau/sum(sau)*100),1))) %>%
          ggplot(aes(x = factor(zonage_simple), y = perc_sau, fill = factor(evol_med),label=paste(perc_sau,"%",sep=""))) + facet_wrap(~period) +
          scale_fill_manual(values=couleur,name="Evolution des teneurs en CO")+
          geom_bar(stat="identity", width = 0.7) +
          geom_text(size = 3, position = position_stack(vjust = 0.5))+
          xlab("Principales régions d'élevage")+ylab("Pourcentage de SAU (%)")+
          theme_perso() + labs(title="b") + coord_flip()

tt <- do.call(grid_arrange_shared_legend,c(p2,list(nrow=2,ncol=1,bottom="",left="")))
ggsave(tt,file = paste(repsortie,"diff_canton_histo_sau.png",sep=""), width = 11, height = 9)  
ggsave(tt,file = paste(reppapier,"diff_canton_histo_sau.png",sep=""), width = 11, height = 9)  

# Test sur les différences relatives pondérées par la SAU (26/06/17) 
#names(bdatdiff)
#bdatdiff <- subset(bdatdiff,evol_med!="Non significatif")
#test <- bdatdiff %>% 
          #group_by(zonage_simple) %>% 
          #summarise(sau=sum(sau_2010,na.rm=TRUE),count=n(),diff=median(diff_med_pourcent,na.rm=TRUE)) %>% 
          #mutate(perc=(round((count/sum(count)*100),1)),perc_sau=(round((sau/sum(sau)*100),1)),diff_sau=(diff/sau)*100000)
#test <- as.data.frame(test)[,c("zonage_simple","diff","diff_sau")]
```
...
for(i in c("13","15","35")){
  cpt <- cpt + 1
  datatest <- bdatdiff[bdatdiff$zonage_simple !="H" & bdatdiff$period %in% i,]

##
  datatest <- datatest[complete.cases(datatest[occupsol]),] 
  wilcotest <- generate_label_df(db=datatest,value="diff_med_pourcent",lev=occupsol,letters,position=1)

  p[[i]] <- ggplot(datatest)+ geom_boxplot(aes_string(y="diff_med_pourcent",x=occupsol,fill="zonage_simple"))+ geom_hline(yintercept = 0,color="red") + scale_fill_manual(values=couleur,name="test") + geom_text(data = wilcotest, vjust=-0.75,aes(x = plot.labels, y = V1, label = paste(labels,"/",Nbr_analyse,sep=""),size=5)) +theme_perso() + theme(legend.position="none") + labs(title=i,x = NULL, y = NULL) + ylim(-50, 155)
}
...
Production de plusieurs figures pour illustrer les évolutions globales du CO et les évolutions de l'occupation du sol

```{r,eval=TRUE,echo=FALSE,results='asis',fig.height=8, fig.width=8,fig.align="center"}
melted.9014 <- sqlQuery(loc,paste("select * from dm_traitements.melted_bdat_9014",sep=""),stringsAsFactors=TRUE,as.is=TRUE)
melted.9014$value <- as.numeric(melted.9014$value)
melted.9014 <- melted.9014[complete.cases(melted.9014$value) & melted.9014$variable %in% "med_corgox",]
melted.9014$annees <- factor(melted.9014$annees,levels=c("9094","9599","0004","0509","1014"))
levels(melted.9014$annees) <- c("1990-1994","1995-1999","2000-2004","2005-2009","2010-2014")
tt <- list()
for(i in reg_elevage){
  melted_focus <- melted.9014[melted.9014$zonage_simple %in% i,]

  # Test statistiques
  wilcotest <- generate_label_df(db=melted_focus,value="value",lev="annees",letters,position=1)
  ...
  p_bdat <- ggplot(melted_focus,aes(x=annees,y=value)) + 
                 ...
                 theme_classic()+theme_perso("") + labs(title=i)
                 geom_text(data = wilcotest, vjust=-0.75,aes(x = plot.labels, y = V1, label = paste(labels,"/",Nbr_analyse,sep=""))) #+ coord_cartesian(ylim = c(14,15))

  colors <- brewer.pal(3,"Set1")
  melted.ra_focus <- melted.ra[melted.ra$annees %in% c("1970","1979","1988","2000","2010") & melted.ra$variable %in% c("p_c","p_mf","p_prairie") & melted.ra$zonage_simple %in% i,]
...
ggsave(tt,file = paste(repsortie,"med_corgox_RA_reg",i,".png",sep=""), width = 8, height = 7) 
}

#France entière#
melted.ra_focus <- melted.ra

p_bdat <- ggplot(melted_focus,aes(x=annees,y=value)) + 
          geom_boxplot() +
          xlab("") +
          stat_summary(fun.y=mean, geom="line",size = 1.5, aes(group=1)) +
          #geom_smooth(aes(x=as.integer(annees),y=value,group=1),color="black",method=loess)+
          ...
          theme(
                ...
                legend.position = "bottom") + labs(title="France")#+
...
last_vals <- c(34,8.5,41)
occupsol <- c("Céréales","Maïs-fourrage","Prairies")
names(colors) <- occupsol
names(last_vals) <- occupsol

gg <- p_ra + theme(legend.position="none")
#gg <- p_ra + theme(plot.margin = unit(c(1, 7, 2, 1), "lines"))

cpt <- 0
for (i in occupsol) {
  cpt <- cpt + 1
  gg <- gg + annotation_custom(grob=textGrob(i, hjust=0,
                                             gp=gpar(fontsize=10,
                                                     face="bold",
                                                     col=colors[cpt])),
                               xmin=5, xmax=5,
...
ggsave(tt,file = paste(repsortie,"co_occup.png",sep=""), width = 8, height = 7) 
...
# Test en cours cf. remarques de Pierre D.

```{r,eval=TRUE,echo=TRUE}
# Meilleurs co-variables du modèle cubist pour une période donnée
covNames <- test[test$period %in% "9599","covariable"]
Ranthrop <- as.character(test[test$type2 %in% "Anthropique" & test$period %in% "9599","covariable"])
Rnaturel <- as.character(test[test$type2 %in% "Naturelle" & test$period %in% "9599","covariable"])

#dcast.bdat[complete.cases(dcast.bdat[c(as.character(Rnaturel),as.character(Ranthrop),Variay)]),]

reslm <- lm_variatype(dcast.bdat,Rnaturel,Ranthrop,"med_corgox_9599",stepaiclm=FALSE)

print(summary(reslm$lmanthrop))
print(summary(reslm$lmnaturel))
print(summary(reslm$lmcplt))
print(reslm$df)

fitanthrop <- reslm$lmanthrop$fitted.value
fitnaturel <- reslm$lmnaturel$fitted.value
fitcplt <- reslm$lmcplt$fitted.value

dbplot <- dcast.bdat[complete.cases(dcast.bdat[,c("med_corgox_9599",Ranthrop,Rnaturel)]),]
plot(dbplot$med_corgox_9599,dbplot$med_corgox_9599,pch=19,col="black")     
abline(a=0,b=1)     
points(dbplot$med_corgox_9599,fitanthrop,pch=19,col="red",cex=0.7)     
points(dbplot$med_corgox_9599,fitnaturel,pch=19,col="green",cex=0.7)     
points(dbplot$med_corgox_9599,fitcplt,pch=19,col="blue",cex=0.7)     
     legend(     
       x="topleft",     
       legend=c("points originaux","anthrop","naturelles","cplt"),     
