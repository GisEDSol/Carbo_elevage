...
melted.bdat <- melted.bdat[melted.bdat$variable %in% "Médiane",]
```
...
for(i in levels(as.factor(melted.bdat$zonage_simple))){
  melted.test <- melted.bdat[melted.bdat$zonage_simple %in% i & melted.bdat$variable %in% "Médiane",]
...
test1 <- generate_label_df(db=melted.bdat[melted.bdat$variable %in% "Médiane",],value="value",lev="annees",LETTERS,position=1)
...
melted.bdat_regelevage <- melted.bdat[complete.cases(melted.bdat$zonage_simple) & (melted.bdat$zonage_simple %in% reg_elevage) & melted.bdat$variable %in% "Médiane",]
...
cpt <- 0
  for(s in reg_elevage){
    cpt <- cpt + 1
    melted.test <- melted.bdat[(melted.bdat$variable %in% "75% centile") & melted.bdat$zonage_simple %in% s,]
...
## Modélisation des teneurs en CO avec Cubist
...
wilcotest <- generate_label_df(db=datatest,value="diffmedian_corgox",lev=occupsol,letters,position=1)
lablex <- labelx
labley <- "Différences des teneurs en CO (g/kg)"
p <- ggplot(datatest)+ geom_boxplot(aes_string(y="diffmedian_corgox",x=occupsol,fill="zonage_cplt"))+ geom_hline(yintercept = 0,color="red") + scale_fill_manual(values=couleur,name="") +geom_text(data = wilcotest, vjust=-0.75,aes(x = plot.labels, y = V1, label = labels)) + scale_x_discrete(lablex)+scale_y_continuous(labley)+theme_perso() + theme(legend.position="none")
p
```
...
melted.ra_focus <- melted.ra[melted.ra$annees %in% c("1970","1979","1988","2000","2010") & melted.ra$variable %in% c("p_c","p_mf","p_prairie","p_sfp") & melted.ra$zonage_cplt %in% reg_elevage,]
