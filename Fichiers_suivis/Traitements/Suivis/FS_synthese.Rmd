...
Cette synthèse a pour objectif de centraliser tous les traitements réalisés en vue de publier un article sur la dynamique des teneurs en CO à l'échelle de la France. L'ensemble des traitements et figures produites dans cette synthèse seront intégrés dans le papier.
...
Dans la présentation des régions d'élevage, on précisera que l'on travaille uniquement avec un nombre défini de régions. Il faudra définir correctement le critère de sélection de ces régions. POur le moment, on présente les résultats pour les principales régions d'élevage (zonage simple)
...
## Statistiques descriptives (Etat des lieux général)
...
Le tableau XX présente les statistiques des teneurs en Carbone Organique (CO) pour les différentes périodes analyées. La période 2000-2004 montre la valeur médiane la plus faible avec une valeur de `r bdatsummarystats$med_corgox$value["0004","Median"]` g/kg. Les valeurs les plus importantes sont observées pour les périodes 1995-1999 et 2010-2014 avec des médianes de teneurs en carbone organique équivalente, à `r bdatsummarystats$med_corgox$value["9599","Median"]` g/kg. La significacité de ces tendances a été évaluée avec le test de wilcoxon. Ces résultats ainsi que l'évaluation des vitesses d'évolution des teneurs en CO sont présentés dans la figure XX. La baisse des teneurs en CO entre la période 1995-1999 et les périodes 2000-2004 et 2005-2009 sont significatives et sont de 0.8 et 0.3% par an respectivement. Suite à cette baisse, la teneur en CO augmente significativement entre la période 2000-2004 et 2005-2009 et 2010-2014 avec un taux de 0.4 et 0.6% par an respectivement.
...
p[["2"]] <- ggplot(datasummary2, aes(ymin = StartDate, ymax = EndDate, x = Count)) + geom_linerange(aes(colour=value),size=3) + coord_flip() + scale_colour_gradient2(limit = c(min(datasummary2$value),max(datasummary2$value)),low="orange",high=muted("purple"),name="Taux d'évolution annuel (%)") + ylab("Périodes") + xlab("") + theme_perso() + geom_text(aes(fontface=2,y = StartDate, x = Count, label = ifelse(pvalue<=0.05,paste(round(value,1),"% **",sep=""),paste(round(value,1),"%",sep=""))), hjust = 0, vjust = 0.5, size = 3.5)# + labs(title= paste(i))
#}
...
# Ici, on relance la modélisation avec les variables sélectionnées
...
for(i in Variay){
  cpt <- cpt + 1
  varimp <- readRDS(paste(repsortie,"mcubist_fr_",i,"_",nbr,"_bis.rds",sep=""))
...
pander(restbis[,1:3],caption="Moyenne des indicateurs de la validation croisée pour les différents modèles")
#pander(rest[,1:3],caption="Moyenne des indicateurs de la validation croisée pour les différents modèles")
```

Les 4 modèles sont essentiellement construits sur la base de variables d'origine naturelles (voir figure XX). Le taux d'argile, les variables climatiques (notamment l'écart de pluie en janvier) et la topographie occupent une place importante dans la modélsation des teneurs en CO à l'échelle de la France. L'importance de l'occupation du sol est secondaire et est principalement associée au pourcentage de Surface Fourragère Principale (SFP). 

```{r,eval=TRUE,echo=FALSE,message=FALSE,warning=FALSE,error=FALSE,fig.height=9, fig.width=11,fig.align="center"}
toto <- ggplot(dfbis,aes(x=period,y=value,fill=type,label=paste(covariable,":",round(value),"%",sep="")))+geom_col(width = 0.7) + geom_text(size = 3, position = position_stack(vjust = 0.5)) + theme_perso() + xlab("")+ylab("Pourcentage d'importance")
toto
```
...
bdatdiff <- melted.bdatdiff#subset(melted.bdatdiff,evol_med!="I")
...
#ici
bdatdiff <- melted.bdatdiff[melted.bdatdiff$zonage_simple !="H" & melted.bdatdiff$period %in% c("13","15","35"),]
bdatdiff <- subset(bdatdiff,evol_med!="I")
code_regelevage <- read.csv(paste(repmetadonnees,"Nomenclature_regionelevage.csv",sep=""))

couleur <- as.character(code_regelevage[code_regelevage$Zonage_simple %in% reg_elevage,"Code_couleur_simple"])
occupsol <- "zonage_simple"
labelx <- "Grandes régions d'élevage"
datatest <- bdatdiff[bdatdiff$zonage_simple %in% reg_elevage,]
datatest <- datatest[complete.cases(datatest[occupsol]),] 
wilcotest <- generate_label_df(db=datatest,value="diff_med",lev=occupsol,letters,position=1)
lablex <- labelx
labley <- "Différences des teneurs en CO (g/kg)"
p <- ggplot(datatest)+ geom_boxplot(aes_string(y="diff_med",x=occupsol,fill="zonage_simple"))+ geom_hline(yintercept = 0,color="red") + scale_fill_manual(values=couleur,name="test") + geom_text(data = wilcotest, vjust=-0.75,aes(x = plot.labels, y = V1, label = labels)) + scale_x_discrete(lablex)+scale_y_continuous(labley)+theme_perso() + theme(legend.position="none")
p
```
...
# Pour insérer l'image
nomfichier <- c("Median_diff_corgox2.png")
...
for(x in nomdiff$Code){
  ...
  for(i in ra_occupyears){
    ...
    p[[paste(i,x,sep="")]] <- ggplot(datatest,aes_string(y="diff_med",x=i))+geom_boxplot(outlier.colour = NA) + geom_point(aes(colour=zonage_simple),position = position_jitter(width = 0.1),alpha = 0.7) + coord_flip() + geom_hline(yintercept = 0,color="red") + geom_text(data = wilcotest, vjust=-0.75,aes(x = plot.labels, y = V1, label = labels)) + scale_x_discrete(lablex)+scale_y_continuous(labley)+theme_perso()+labs(title=titre)+scale_color_manual(values=colors,name="Région d'élevage")#+theme(axis.text.y = element_text(angle = 45, hjust = 0))
  }
}
...
```{r,eval=FALSE,echo=FALSE,fig.height=9, fig.width=11,fig.align="center",results='asis'}
##TEST
p <- list()
cpt <- 0
for(x in nomdiff$Code){
  ...
  for(i in ra_occupyears){
    ...
    p[[paste(i,x,sep="")]] <- ggplot(datatest,aes_string(y="diff_med",x=i))+geom_boxplot() + facet_wrap(~zonage_simple) + coord_flip() + geom_hline(yintercept = 0,color="red") + geom_text(data = wilcotest, vjust=-0.75,aes(x = plot.labels, y = V1, label = labels)) + scale_x_discrete(lablex)+scale_y_continuous(labley)+theme_perso()+labs(title=titre)#+theme(axis.text.y = element_text(angle = 45, hjust = 0))
  }
}
...
### Modélisation de l'évolution des teneurs en CO
