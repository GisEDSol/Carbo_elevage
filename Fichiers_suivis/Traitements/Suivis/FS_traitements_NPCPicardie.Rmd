...
nomfichier <- c("Median_evolu_c31_22.png","Median_diff_c31_22.png","corgox_31_22.png")
...
for(i in periodetud){
    ...
    save(mgbm_picardiNPC,file=paste(repsortie,"mgbm_picardiNPC_",i,".RData",sep=""))
}
save(p,file=paste(repsortie,"plot_mgbm_picardiNPC.RData",sep=""))
...
ggsave(tt, file = paste(repsortie,"plot_mgbm_picardiNPC.png",sep=""),width = 8, height = 8)  
```
...
Rcovar <- c("var_mf1970_2010","var_mf1979_2010","var_mf1988_2000","var_mf1970_2000","var_sfp1970_2000","var_sfp1988_2000","std_pluie_janv","std_pluie_juil")
...
```{r,echo=FALSE,eval=TRUE,fig.height=7, fig.width=7,fig.cap = fig$cap("Graphs_correl_NPC_picardie","Matrice de correlation"),fig.align="center"}
periodc <- "diffmedian13"
Rcovar <- c("var_mf1979_2010","var_mf1988_2000")
vNames <- c("diffmedian13","diff13",Rcovar)

p <- list()
for(i in Rcovar){
    dataplot <- dcast.bdat[(dcast.bdat$code_reg %in% reg_focus) & complete.cases(dcast.bdat[,vNames]),][,vNames]

    p[[i]] <- ggplot(dataplot, aes_string(i,"diffmedian13")) +
                geom_point(aes(colour = factor(diff13)),alpha = 0.8, size = 1) +
                ...
                annotate("text", x=0, y=5, label=lm_eqn(lm(dataplot[,"diffmedian13"]~dataplot[,i], dataplot)), parse=TRUE)+
...
```{r,echo=FALSE,eval=TRUE,fig.height=7, fig.width=7,fig.cap = fig$cap("boxplot_diffC_occup","boxplot NPC_picardie"),fig.align="center"}
# Temporaire
