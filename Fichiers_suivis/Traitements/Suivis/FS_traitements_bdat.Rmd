...
pander(bdatsummarystats["dqua_corgox"][[1]],caption = "Statistiques descriptives des teneurs en carbone organique pour les différentes périodes")
```
...
test2 <- generate_label_df(db=melted.bdat[melted.bdat$variable %in% "dqua_corgox",],value="value",lev="annees",letters,position=1)
...
Ci-dessous, la figure présente la répartition des teneurs en C pour les différentes périodes et pour les types de climat. Les teneurs en CO sont significativement différentes entre les types de climat et la zone du climat océanique dégradé des plaines du Centre et du Nord présentent les valeurs de CO les plus faibles.
...
test2 <- generate_label_df(db=melted.bdat_prairie[melted.bdat_prairie$variable %in% "dqua_corgox",],value="value",lev="classe_p_prairie1988",letters,position=1)
...
Toutefois, sur l'ensemble des périodes analysées, les différences des teneurs entre les différentes strates géographiques sont systématiquement différentes. 
...
Rcovarstrates <- c("zonage_cplt","zonage_simple","typo_clim","code_dept")#test avec eff_corgco9599
...
i-dessous, les figures présentent l'effet des principales types de variables sur les teneurs en CO dans la modélisation. Les figures montrent des relations quasi-linéaire entre des variables comme l'altitude, la surface fouragère principale, la hauteur de pluie l'argile et la teneur en CO.
...
## Construction des variables occupation du sol en fonction de la période en C étudiée
Variay <- c("med_corgox_9599","med_corgox_0004","med_corgox_0509","med_corgox_1014")
type <- c(typetopo,typepedo,typeclimato)
...
for(i in Variay[1]){
  ...
  saveRDS(mcubist_fr,file=paste(repsortie,"mcubist_fr_",i,"_bestvariable",nbr,".rds",sep=""))
}
###


## ## ## 
varimp <- readRDS(paste(repsortie,"mcubist_fr_",i,"_anthrop",nbr,".rds",sep=""))
qualityindex <- varimp$qualityindex
varimp <- as.data.frame(varimp$varimp)
varimp <- varimp[grep("Overall",names(varimp))]
varimp$median <- apply(varimp, 1, function(x){median(x,na.rm=TRUE)})
