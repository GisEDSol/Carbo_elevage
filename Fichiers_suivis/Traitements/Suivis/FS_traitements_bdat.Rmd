...
melted.bdat <- melted.bdat[melted.bdat$variable %in% c("corgox","corgox_ddec"),]
melted.bdat$variable <- ordered(melted.bdat$variable, levels = c("corgox","corgox_ddec"))
levels(melted.bdat$variable) <- c("Médiane","75% centile")
#melted.bdat$variable <- ordered(melted.bdat$variable, levels = c("corgox_pdec","corgox_pqua","corgox","corgox_dqua","corgox_ddec"))
#levels(melted.bdat$variable) <- c("10% centile","25% centile","Médiane","75% centile","90% centile")
```
...
> **Compte-tenu du manque de donnée observée dans la première période 9094, celle-ci ne sera pas prise en compte dans les analyses des teneurs en CO à l'échelle de la France.** 
...
## Préparation des données (en cours, test et questionnement sur l'intérêt)

```{r echo=FALSE,eval=FALSE,warning=FALSE,error=FALSE}
dcast.bdat

melted.test <- melted.bdat[melted.bdat$variable %in% "Médiane",]
melted.test$winsorize <- winsorize(melted.test[,"value"])
melted.test$diff <- melted.test$winsorize-melted.test$value
outliers_canton <- unique(as.character(melted.test[melted.test$diff!=0,"code_canton"]))

melted.test <- melted.bdat[melted.bdat$variable %in% "Médiane",]
# Dans l'idéal, il serait intéressant d'identifier les zones équettées...Quid de la Bretagne, Haut de France ?
...
# Revoir la discussion sur les outliers (https://www.researchgate.net/post/Should_outliers_be_removed_before_or_after_data_transformation)
# Sinon, on peut supprimer les outliers et regarder l'effet sur la dynamique globale dans l'évolution des teneurs en CO.? et comparer avec les valeurs initiales.?

test <- outlier(test)apply(melted.test["value"],2, function(x) tapply(x, melted.test["annees"],outlier))



melted.test$logvalue <- log(melted.test$value)
shapirobdat <- apply(melted.test["logvalue"],2, function(x) tapply(x, melted.test["annees"],shapiro.test))
# Avec ou sans le log, la distribution des valeurs n'est pas normale, autant conserver les vrais valeurs plutôt que les logs.
...
map <- dbReadSpatial(con, schemaname="dm_vecteurs", tablename="canton_9514", geomcol="geom")
data <- map@data
...
for(i in levels(as.factor(melted.bdat$typo_clim))){
  melted.test <- melted.bdat[melted.bdat$typo_clim %in% i & melted.bdat$variable %in% "Médiane",]
...
melted.bdat_clim <- melted.bdat[(melted.bdat$variable %in% "Médiane") & (melted.bdat$typo_clim %in% typo_clim),]
...
test1 <- generate_label_df(db=melted.bdat_prairie[melted.bdat_prairie$variable %in% "Médiane",],value="value",lev="classe_p_prairie1988",LETTERS,position=1)
...
for(i in Variay){
  ...
  pourcent <- list()
  for(s in strates){
    modelcubistrds <- readRDS(paste(repsortie,"mcubist_strates_",s,"_",i,"_",nbr,"2.rds",sep=""))
