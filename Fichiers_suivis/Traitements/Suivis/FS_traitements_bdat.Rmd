...
p_zoom <- ggplot(melted.bdat[(melted.bdat$value > 17) & (melted.bdat$value < 45) ,], aes(x=annees,y=value)) + 
  #geom_violin(trim=FALSE)+
  #geom_boxplot(width=0.1, fill="white")+
  geom_boxplot()+
  #scale_color_manual(values=colors,name="Années")+
...
grid.arrange(p,p_zoom,nrow=1,ncol=2)
...
nomfichier <- "effectif_bdat" #Nom du fichier
...
id <- "id_geofla"# Nom du champs présentant un identifiant unique
classe_valeur <- classIntervals(d[,1],n=5,style="quantile",digits=2,na.rm=TRUE)[[2]]
d[,"classeC"] <- cut(d[,1] ,breaks = data.frame(classe_valeur)[,1],include.lowest=T)  
...
p <- list()
for(i in levels(d[,"classeC"])){
  print(i)
  d_classC <- d[d$classeC %in% i,]
  trainrows <- runif(nrow(d)) > 0.3
  ...
  mgbm_fr_classC <- try(train(formula,data=train,"gbm",tuneGrid = tuneGrid,trControl = trControl,verbose = F,keep.data = T,na.action = na.omit),silent=TRUE)

  if(class(mgbm_fr_classC)[1]=="try-error"){
    print("Pas assez d'échantillons pour la modélisation")
    next
  }

  varimport <- varImp(mgbm_fr_classC)
...
save(p,file=paste(repsortie,"p_mgbm_classeC.RData",sep=""))
...
```{r,highlight=TRUE,eval=TRUE,echo=FALSE,fig.height=10, fig.width=10,fig.cap = fig$cap("gbm_classeC_fr","Importance des variables explicatives"),fig.align="center"}
load(paste(repsortie,"p_mgbm_classeC.RData",sep=""))
