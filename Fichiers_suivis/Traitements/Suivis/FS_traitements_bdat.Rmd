...
carto(dsn,tablecarto,variablecarto,nclasse,style_classe,couleur,l_legend=l_variable,repsortie,nomfichier,title=c("1995-1999","2000-2004","2005-2009","2010-2014"),caption="",dept=FALSE,reg=FALSE,nrowlayout=2,ncollayout=2,position="bottom",ggsaveheight=7,ggsavewidth=6.27)
```
...
tx_baisse_fr <- round(((bdatsummary$value["9599","Median"]-bdatsummary$value["0004","Median"])/bdatsummary$value["9599","Median"])*100,1)
tx_augmentation_fr <- round(((bdatsummary$value["1014","Median"]-bdatsummary$value["0004","Median"])/bdatsummary$value["1014","Median"])*100,1)
```
...
couleur <- brewer.pal(nclasse,"Paired")#nom de la palette couleur (selon RColorBrewer)display.brewer.all() pour connaître les différentes palettes
...
```{r,highlight=TRUE,eval=TRUE,echo=FALSE,fig.height=6, fig.width=12,fig.cap = fig$cap("boxplot_regclim",""),fig.align="center"}
...
p <- ggplot(melted.bdat_clim) +
            geom_boxplot(aes(x=typo_clim,y=value))+
            #geom_smooth(aes(x=as.integer(annees),y=value,color=zonage_simple,fill=zonage_simple),method=loess)+
            #scale_color_manual(values=colors,name="Années")
            scale_x_discrete("Type de climat")+scale_y_continuous("Teneur en carbone (g/kg)")+
            theme_perso()
...
```{r, eval=TRUE,echo=FALSE}
pairwise.wilcox.test(melted.bdat_clim[,"value"], melted.bdat_clim[,"typo_clim"])
```
...
nclasse <- 8 #Nombre de classes de valeurs pour la cartographie
style_classe <- "fixed" #"pretty"#"jenks","fixed"
couleur <- c(brewer.pal(nclasse,"Paired"),"#000010") #nom de la palette couleur (selon RColorBrewer)
...
p <- ggplot(melted.bdat_regelevage) +
            geom_boxplot(aes(x=zonage_cplt,y=value))+
            #geom_smooth(aes(x=as.integer(annees),y=value,color=zonage_simple,fill=zonage_simple),method=loess)+
            #scale_color_manual(values=colors,name="Années")
            scale_x_discrete("Région d'élevage")+scale_y_continuous("Teneur en carbone (g/kg)")+
            theme_perso()
...
for(i in Variay){
  ...
  # Compte tenu du poid de ces objets, on recueille uniquement qualityindex et varimport
  mcubist_fr <- mcubist_fr[c("qualityindex","varimport")]
...
for(i in Variay){
  ...
  modelcubistrds <- readRDS(paste(repsortie,namemodel,".rds",sep=""))
  p[[cpt]] <- ggplot(modelcubistrds$varimport, aes(x = variable, y = importance,fill=type)) + 
              geom_bar(stat = "identity") + coord_flip() + labs(title=i)
  rest[cpt,"r2"] <- modelcubistrds$qualityindex[,"R2"][[1]]
  rest[cpt,"MSE"] <- modelcubistrds$qualityindex[,"MSE"][[1]]
  rest[cpt,"RMSE"] <- modelcubistrds$qualityindex[,"RMSE"][[1]]
  impvar[[i]] <-modelcubistrds$varimport
}
```
