...
melted.bdat <- sqlQuery(loc,paste("select value,variable,annees,",variafactor," from dm_traitements.melted_bdat",sep=""),as.is=c(FALSE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE))
...
```{r ,highlight=TRUE,eval=TRUE,echo=FALSE,warning=FALSE,message=FALSE,error=FALSE}
# Graphique général
...
```{r boxplot_periodes,highlight=TRUE,eval=TRUE}
...
```{r boxplotbdat_classeoccup,highlight=TRUE,eval=TRUE}
for(i in id_class){
...
cartoperiod(dsn,tablecarto,variablecarto,nclasse,style_classe,couleur,l_legend,repsortie,nomfichier,dept=FALSE,reg=FALSE,nrowlayout=2,ncollayout=3,position="bottom")
```
