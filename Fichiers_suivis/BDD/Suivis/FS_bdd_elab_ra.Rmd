...
Revoir les données UGBta, il y a très peu de différence entre 1988 et 2010. NOrmal ou non?
...
p_variable <- c("sth","cereale","sfp","mf")
periodref <- "2010"
period <- c("1970","1979","1988")
...
for(v in variable){
  ...
  for(p in period){
	  ...
	  varia <- paste("var_",p_variable[cpt],p,"_",periodref,sep="")
	  ...
	  sqlQuery(loc,paste("alter table ",table_dm,"
	                      ...
	                      update ",table_dm,"
     	                	SET ",varia," = s1.",varia," from(
                      	select ((cc.",v," - tt.",v,")/",SAU,")*100 as ",varia,",tt.num_canton
                      	from ",schema,".",type_RA,p," as tt
                      	right join ",schema,".",type_RA,periodref," as cc on cc.num_canton=tt.num_canton::text) as s1
                      	where ",table_dm,".code_canton=s1.num_canton::text",sep=""))
    
	  # Ajout d'un commentaire sur la nouvelle colonne crée
	print(sqlQuery(loc,paste("
		COMMENT ON COLUMN ",table_dm,".",varia," IS \'Evolution ",signification[cpt]," entre ",periodref," et ",p," par rapport à la SAU de ",p,".\';",sep="")))
	}
}
```
...
# Calcul de l'évolution des superficies pour 1970-2010;1979-2010;1988-2010
# Paramètres
p_variable <- c("sth","c","sfp","mf","prairie")
periodref <- "2010"
period <- c("1970","1979","1988")
signification <- cbind("de la surface toujours en herbe","de la surface en céréales"," de la surface fourragère principale","de la surface en maîs fourrage ensilage","prairie")
type_RA <- "s_cultures_canton"
table_dm <- "dm_vecteurs.canton" # Nom de la table pour rassembler les calculs (vers le schéma dm_vecteurs)
...
for(v in p_variable){
	cpt <- cpt + 1
  for(p in period){
    varia <- paste("var2_",v,p,"_",periodref,sep="")
  	  
  	variacalA <- paste("p_",v,periodref,sep="")   
	  ...
	  # Ajout d'un commentaire sur la nouvelle colonne crée
	print(sqlQuery(loc,paste("
		COMMENT ON COLUMN ",table_dm,".",varia," IS \'Evolution ",signification[cpt]," entre ",periodref," et ",p," par rapport à la SAU de ",p," (occupA/SAUA)-(occupB/SAUB).\';",sep="")))
	}
}
```
...
# Paramètres
p_variable <- c("sth","c","sfp","mf","prairie")
period <- c("1970_2010","1979_2010","1988_2010")
...
### Occupation du sol
Problème avec les données ugbta, 
```{r,highlight=TRUE,eval=FALSE}
# Paramètres
varia <- c("p_sth","p_sfp","p_prairie","p_c","ugbta") # Variable à analyser 
