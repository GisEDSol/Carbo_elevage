...
```
### Importation des résultats pour le 3ème quartile

```{r importBDAT_harmomethodes, eval=FALSE}
#Attention, changer la valeur de la virgule avant d'importer le fichier...
ListBDAT <- gsub("\\.csv$","",list.files(paste(repBDAT,"BDAT_harmonisee/tables_3emequartile/",sep=""),pattern="\\.csv$"))
  for(i in ListBDAT){
    print(i)
    ...
    # Lecture du fichier
    txtBDAT <- read.csv(paste(repBDAT,"BDAT_harmonisee/tables_3emequartile/",i,".csv",sep=""),sep=";",header=TRUE,colClasses=c("Canton"="character"))

    colnames(txtBDAT)[1] <- "canton"
    colnames(txtBDAT)[c(1,4,5,6,7,9,10)] <- c("canton","dqua","dqua2",paste("diffdqua",comp,sep=""),"diff","eff","effp2")
    txtBDAT <- txtBDAT[,c(1,4,5,6,7,9,10)]
    head(txtBDAT)
...
corgox_dqua
  ...
  med2 double precision,
  diffmedian12 double precision
