...
La liste d'instructions suivante intègre les analyses des teneurs de la BDAT ré-échantillonnées dans la base de données. Ci-dessous, ce sont les analyses provenant des deux méthodes de mesures du carbone qui sont intégrées (oxydation humide - corgox +combustion sèche - corgco).
...
Ci-dessous, les analyses des teneurs en C classées par méthode de mesure sont intégrées dans la base de données. Les analyses réalisées avec la méthode oxydation humide sont nommée `corgox` et les analyses mesurées avec la méthode combustion sèche `corgco`.

```{r importBDAT_harmomethodes, eval=FALSE}
# Selection des fichiers à importer
repmethodo <- c("Oxydationhumide","Combustionseche")
for(r in repmethodo){
  ListBDAT <- gsub("\\.csv$","",list.files(paste(repBDAT,"BDAT_harmonisee/",r,"/",sep=""),pattern="\\.csv$"))
  for(i in ListBDAT){
    print(i)
    # Lecture du fichier
    txtBDAT <- read.csv(paste(repBDAT,"BDAT_harmonisee/",r,"/",i,".csv",sep=""),sep=";",header=TRUE,colClasses=c("canton"="character"))
...
La base de données européenne des sols a été téléchargée sur ce [site](http://esdac.jrc.ec.europa.eu/content/european-soil-database-v20-vector-and-attribute-data#tabs-0-description=1&tabs-0-description-2=). 
...
Pour les métadonnées de la BDGSF, [voir](http://eusoils.jrc.ec.europa.eu/ESDB_Archive/ESDBv2/fr_easy.htm)
...
system(paste("shp2pgsql -s 2154 -c -D -W \"latin1\" -I ",file2," ",schema,".",sortie,"| psql -d sol_elevage -h localhost -U jb",sep=""))

bdgsf <- sqlQuery(loc,"select * from bdgsf.sgdb limit 1")
n_bdgsf <- names(bdgsf)[-length(bdgsf)]
n_bdgsf <- paste(n_bdgsf,collapse=",")

# Intersection avec postgis
sqlQuery(loc,paste("CREATE TABLE ",schema,".",sortie,"_fr AS
            select a.",n_bdgsf,",st_intersection(a.geom, b.geom) as geom
            from bdgsf.sgdb as a, dm_vecteurs.departement as b
...
for(i in list.rast){
    ...
    rpostgisname <- paste("dm_rasters.fr_",i,".tif",sep="")
...
Les données du recensement agricole ont été extraites de requêtes lancées sur [Disard](https://stats.agriculture.gouv.fr/disar/). 
Dans les tables de sorties de Disard, le numéro de canton et le nom du chef lieu sont enregistrés dans le même champ. Dans les commandes suivantes, le numéro de canton et le nom du chef lieu sont dissociés dans un champ qui leur est propre pour faciliter les jointures futures.  
...
Le raster est ensuite intégré sur le serveur PostGIS (revoir, problème d'intégration)
...
```{r,eval=FALSE}
# Export avec pg_dump
pg_dump sol_elevage > sol_elevage.sql
