...
for(i in vpedo){
  ...
  # Ajout d'un commentaire sur la nouvelle colonne crée
  print(sqlQuery(loc,paste("
  COMMENT ON COLUMN ",table_dm,".",i," IS \'Médiane du taux d\''argile (g/kg) calculée sur 1990-2009.\';",sep="")))
}
...
Ici, la table `dm_vecteurs.canton` est dupliquer et filtrer pour un travail basé uniquement sur un nombre de canton ayant des analyses sur l'ensemble de période étudiée. Cette table d'exploitation est nommée `dm_vecteurs.canton_cplt`. Elle servira de base pour la création de la table au format long complète également.
**Nota. Changement de stratégie. On utilisera deux types de masques :

- canton_cplt pour toutes les périodes
- canton_9504 pour l'exclusion de la première période

## Format large

```{r,highlight=TRUE,eval=FALSE}

sqlQuery(loc,"DROP TABLE IF EXISTS dm_vecteurs.canton_cplt")
sqlQuery(loc,"CREATE TABLE dm_vecteurs.canton_cplt as
SELECT *
FROM dm_vecteurs.canton
where corgox_medequi9094 is not null and corgox_medequi9599 is not null and corgox_medequi0004 is not null
and corgox_medequi0509 is not null and corgox_medequi1014 is not null
")

sqlQuery(loc,"DROP TABLE IF EXISTS dm_vecteurs.canton_9504")
sqlQuery(loc,"CREATE TABLE dm_vecteurs.canton_9504 as
SELECT *
FROM dm_vecteurs.canton
where corgox_medequi9599 is not null and corgox_medequi0004 is not null
and corgox_medequi0509 is not null and corgox_medequi1014 is not null
")
...
```{r,highlight=TRUE,eval=FALSE}
# Paramètres
table_dm <- "dm_vecteurs.canton_cplt" # Nom de la table pour rassembler les calculs (vers le schéma dm_vecteurs)
varia <- "corgox_medequi" # Variable à analyser 
period <- c("9094","9599","0004","0509","1014")
variaoccup <- c("p_sth","p_sfp","p_prairie","p_cop","p_c")
#id_class <- apply(expand.grid(varia,period), 1, paste, collapse="")
...
# Enregistrement dans le schema dm_traitements  
tablename <- paste("dm_traitements.","melted_bdat_cplt",sep="")
sqlQuery(loc,paste("drop table if exists ",tablename,sep=""))
sqlSave(loc,melted.bdat,tablename=tablename)
