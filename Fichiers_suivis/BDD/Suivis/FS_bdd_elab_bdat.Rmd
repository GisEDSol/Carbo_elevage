...
## Intégration de la médiane, du 1er et 3ème quartile des teneurs en carbone organique par canton (teneurs + effectifs)
...
cpt <- 0
  for(s in stats){
      cpt <- cpt + 1
  var_variable <- paste("corg_",s,"equiv",i,sep="")
  ...
  COMMENT ON COLUMN ",table_dm,".",var_variable," IS \'",commentstats[cpt]," des teneurs en carbone organique après ré-échantillonnage pour la période (toutes méthodes confondues)",i,".\';",sep="")))
  }
}
```
...
period <- "9009"
schema <- "bdat"
vpedo <- c("argi_med","sabt_med") #Voir pour le pH
...
for(i in vpedo){
  ...
  print(sqlQuery(loc,paste("
  COMMENT ON COLUMN ",table_dm,".",i," IS \'",signification[cpt]," (g/kg) calculée sur 1990-2009.\';",sep="")))
  }  
```
