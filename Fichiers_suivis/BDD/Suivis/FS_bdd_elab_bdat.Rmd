...
schema <- "bdat"
stats <- c("med","dqua")
commentstats <- c("Médiane","3ème quartile")
...
for(i in period){
  ...
  for(x in test){
    cpt <- cpt + 1
    print(cpt)
    toto <- sqlQuery(loc,paste("select med_",i," as med_",i,"_",cpt,",canton
      from bdat.corgox_",stat,"_",x,"_canton",sep=""))
  if(cpt==1){
    corgoxtable <- toto
    }else{
  ...
  var_variable <- paste("med_corgox_",i,sep="")
  ...
  #COMMENT ON COLUMN ",table_dm,".",var_variable," IS \'""teneurs en carbone organique après ré-échantillonnage pour la période ",i,".\';",sep="")))
}
```
...
```
A la suite de cette intégration, les différences relatives entre les différentes périodes sont calculées selont la formule suivante : ((period2-period1])/period1)*100

```{r,highlight=TRUE,eval=FALSE}
## En cours (06/04/17) ## Voir avec le travail un peu plus bas similaire. Attendre les résultats envoyés par Laetitia. Revoir l'organisation de ces données.
...
varia_eff <- c("eff_coox","eff_corgox","eff_corgco") # Variable à analyser test
variaCO <- c("corgox","corgco")
stats <- c("pdec","pqua","dqua","ddec")
varia_stats <- apply(expand.grid(variaCO,stats),1, function(x){paste(x[1],"_",x[2],sep="")})
varia_stats <- c(varia_stats,"corg_pdecequiv","corg_pquaequiv","corg_medequiv","corg_dquaequiv","corg_ddecequiv")
...
periodoccup <- c("1970","1979","1988","2000","2010")
id_class <- apply(expand.grid(variaoccup,periodoccup),1, function(x){paste("classe_",x[1],x[2],sep="")})
id <- c("code_canton","code_reg","nom_region","typo_clim","zonage_simple","zonage_cplt","classe_altimean","mat11","classe_sabt_med","classe_argi_med",id_class) #Nom des identifiants
...
periodref_ra <- c("1988","2010","2000")#c("2010","2000","1988")
...
varia_eff <- "eff_corgox" # Variable à analyser test
variaCO <- "corgox"
stats <- c("pdec","pqua","dqua","ddec")
varia_stats <- apply(expand.grid(variaCO,stats),1, function(x){paste(x[1],"_",x[2],sep="")})
varia <- c(varia_stats,varia_eff,variaCO)
...
melted.bdat <- melt(data=melt.canton,id.vars=c(id,"annees"))
melted.bdat$annees <- as.character(melted.bdat$annees)
melted.bdat$code_canton <- as.character(melted.bdat$code_canton)
melted.bdat$typo_clim <- as.factor(melted.bdat$typo_clim)
